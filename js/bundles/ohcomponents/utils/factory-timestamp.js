 ...
 * @param {Function} updateCache An update function
 * @param {Function} updateArguments Arguments handed over to the update function
 */
export function fromCache(url, cacheTime, updateCache, updateArguments) {
    ...
    if (cachedData && cacheTime > Date.now()) {
        return Promise.resolve(html`<div class="root" innerHTML="${cachedData}"></div>`);
    }
    return new Promise((accept, reject) => {
        updateCache(url, updateArguments).then(parsedHtml => {
            ...
            setTimeout(() => accept(html`<div class="root" innerHTML="${parsedHtml}"></div>`), 1);
        }).catch(e => reject(e));
    });
}