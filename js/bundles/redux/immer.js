...
function get$1(state, prop) {
    ...
    if (state.modified) {
        ...
        if (value === state.base[prop] && isDraftable(value))
            ...
            return state.copy[prop] = createDraft$1(value, state);
        return value;
    } else {
        if (has(state.drafts, prop)) return state.drafts[prop];
        var _value = state.base[prop];
        if (!isDraft(_value) && isDraftable(_value)) return state.drafts[prop] = createDraft$1(_value, state);
        return _value;
    }
}
