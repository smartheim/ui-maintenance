...
var platform_dom$2 = {
  getStyle() {
    ...
    console.log("getStyle", this._style);
    return this._style;
  },
  ...
  _enabled: typeof window !== 'undefined' && typeof document !== 'undefined',
	 ...
	 * @private
	 */
  _ensureLoaded: function (dom) {
    if (this._loaded) {
      return;
    }

    this._style = dom.querySelector("#chartjs");
    this._loaded = true;
  },
...
var adapter = core_adapters._date;
...
helpers$2.cancelAnimFrame = (function () {
  if (typeof window !== 'undefined') {
...
helpers$2.startFrameRefreshTimer = function (context, func) {
  if (!context.frameRequestID) {
    const frameRefresh = function () {
      func();
      context.frameRequestID = helpers$2.requestAnimFrame.call(window, frameRefresh);
    };
    context.frameRequestID = helpers$2.requestAnimFrame.call(window, frameRefresh);
  }
};

helpers$2.stopFrameRefreshTimer = function (context) {
  const frameRequestID = context.frameRequestID;

  if (frameRequestID) {
    helpers$2.cancelAnimFrame.call(window, frameRequestID);
...
var valueOrDefault$d = helpers$2.valueOrDefault;
...
function parse$1(input, scale) {
  if (helpers$2.isNullOrUndef(input)) {
    return null;
  }
...
function resolveOption(scale, key) {
  ...
  return valueOrDefault$d(realtimeOpts[key], streamingOpts[key]);
}
  ...
  chart.data.datasets.forEach(function (dataset, datasetIndex) {
    ...
    if (id === meta.xAxisID || id === meta.yAxisID) {
      ...
      datasetPropertyKeys.forEach(function (key) {
        if (dataset.hasOwnProperty(key) && helpers$2.isArray(dataset[key])) {
...
function scroll(scale) {
  ...
  helpers$2.each(chart.data.datasets, function (dataset, datasetIndex) {
    meta = chart.getDatasetMeta(datasetIndex);
    if (id === meta.xAxisID || id === meta.yAxisID) {
      elements = meta.data || [];
...
var RealTimeScale = TimeScale.extend({
  ...
  update: function () {
    ...
    if (resolveOption(me, 'pause')) {
      helpers$2.stopFrameRefreshTimer(realtime);
    } else {
      helpers$2.startFrameRefreshTimer(realtime, function () {
        scroll(me);
      });
  ...
  draw: function (chartArea) {
    ...
    helpers$2.canvas.clipArea(context, clipArea);
    TimeScale.prototype.draw.apply(me, arguments);
    helpers$2.canvas.unclipArea(context);
  },

  destroy: function () {
    var me = this;

    helpers$2.stopFrameRefreshTimer(me.realtime);
  ...
  _getTimeForIndex: function (index, datasetIndex) {
    ...
    if (helpers$2.isNullOrUndef(time)) {
      value = me.chart.data.datasets[datasetIndex].data[index];
      if (helpers$2.isObject(value)) {
        time = parse$1(me.getRightValue(value), me);
      } else {
        time = parse$1(timestamps.labels[index], me);
      }
    }
...
scaleService.registerScaleType('realtime', RealTimeScale, defaultConfig$5);
 ...
 */
function updateChartData(chart$1) {
  var animationOpts = chart$1.options.animation;
  var datasets = chart$1.data.datasets;
  var newControllers = chart$1.buildOrUpdateControllers();
  var lastMouseEvent = chart$1.streaming.lastMouseEvent;

  datasets.forEach(function (dataset, datasetIndex) {
    chart$1.getDatasetMeta(datasetIndex).controller.buildOrUpdateElements();
  });
  chart$1.updateLayout();
  if (animationOpts && animationOpts.duration) {
    helpers$2.each(newControllers, function (controller) {
      controller.reset();
    });
  }
  chart$1.updateDatasets();

  if (chart$1.animating) {
    // If the chart is animating, keep it until the duration is over
    chart.animationService.animations.forEach(function (animation) {
      if (animation.chart === chart$1) {
        chart$1.render({
    ...
    datasets.forEach(function (dataset, datasetIndex) {
      chart$1.getDatasetMeta(datasetIndex).controller.transition(1);
    });
  }

  if (chart$1.tooltip._active) {
    chart$1.tooltip.update(true);
  }

  if (lastMouseEvent) {
    chart$1.eventHandler(lastMouseEvent);
  }
}
...
var streamingPlugin = {
  ...
  afterInit: function (chart$1) {
    if (chart$1.resetZoom) {
      chart.Zoom.updateResetZoom(chart$1);
    }
  },
  ...
  afterUpdate: function (chart, options) {
    ...
    if (pause) {
      helpers$2.stopFrameRefreshTimer(streaming);
    } else {
      helpers$2.startFrameRefreshTimer(streaming, function () {
        drawChart(chart);
      });
    }
  },
  ...
  beforeDatasetDraw: function (chart, args) {
    ...
    helpers$2.canvas.clipArea(chart.ctx, clipArea);
    return true;
  },

  afterDatasetDraw: function (chart) {
    helpers$2.canvas.unclipArea(chart.ctx);
  },
 ...
 * 
 * @category Web Components
 * @module ui-time-graph
 */

export default Chart$1;
